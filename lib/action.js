...
module.exports = class Action {
  ...
  download (fd, done) {
    ...
    const scope = local(this)
        , params = merge({ Key: fd }).and(scope.opts.request).into({})
  ...
  stream (fd) {
    ...
    const scope = local(this)
        , params = merge({ Key: fd }).and(scope.opts.request).into({})
        , request = scope.client.getObject(params)
  ...
  list (dirname, done) {
    ...
    const scope = local(this)
        , params = merge({ Prefix: dirname }).and(scope.opts.request).into({})
        , objects = []
  ...
  url (operation, done) {
    ...
    return done ? scope.client.getSignedUrl(operation, params, done)
