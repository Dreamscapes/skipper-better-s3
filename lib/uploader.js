...
module.exports = class Uploader extends WritableStream {
  ...
  _write (inStream, encoding, done) {
    ...
    // If not, this is probably a standard http upload stream
    ...
    const name = inStream.path ? path.parse(inStream.path).base
                               : inStream.fd || uuid.v4()
        , type = mime.lookup(name)
        , scope = local(this)
        , fd = scope.opts.dirname ? `${scope.opts.dirname}/${name}`
                                  : name
        ...
        , request = scope.client.upload(params, scope.opts.options, (err, data) => {
          ...
          // For non-http uploads, the fd is not available anywhere, so let's put it to the extra
